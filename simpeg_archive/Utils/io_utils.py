...
def readUBCmagneticsObservations(obs_file):
    ...
    """
    from simpeg_archive.PF import BaseMag
    fid = open(obs_file, 'r')
...
def readUBCgravityObservations(obs_file):
    ...
    """
    from simpeg_archive.PF import BaseGrav
    fid = open(obs_file, 'r')
...
def writeVectorUBC(mesh, fileName, model):
    """
        Writes a vector model associated with a simpeg_archive TensorMesh
...
def readVectorUBC(mesh, fileName):
    """
        Read a vector model associated with a simpeg_archive TensorMesh
        to a UBC-GIF format model file.
        ...
        if inducing_field is not None:
            ...
            if limits is None:
                ...
                if inducing_field.shape[0] == 3:
                    ...
                    # now sort the data to simpeg_archive format
